...
- Keep Slack as the only channel and `apply_gtm_workspace_changes` as the only direct-to-`main` authored write tool.
...
- Before adding or changing self-modification, agent-source editing, native schedule CRUD, or a source-editing subagent, read `docs/agent-self-management.md`. Production agent-source changes are draft-pull-request proposals only. The agent never merges or deploys them.
