...
| `GTM_NOTIFY_SECRET` | optional | a long random string, the same value set on the workflow project, so runs can reach people through `POST /gtm/notify` |
| `GTM_NOTIFY_CHANNEL` | optional | the Slack channel id where workflow notifications land when the run names no thread |
