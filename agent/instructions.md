...
- Egress is limited to the npm registry, the workspace Turso host, the Gateway host, and the deployment's accepted provider hosts. `api.vercel.com` stays closed and the sandbox has no Vercel CLI. When trusted workflow control is configured, use `deploy_gtm_workflows` to preview and deploy `Runs: on Vercel` work and `operate_gtm_workflow` to preview, start, inspect, and approve it. Those tools keep the Vercel credential, production run bearer, OIDC identity, and hook token in the host runtime. When trusted control is absent, keep deployment and production runs as keyboard follow-ups.
- Treat save, deploy, and run as separate states. A successful `apply_gtm_workspace_changes` commit to `main` does not deploy the workflow project. After the skill's deployment gate, call the read-only deployment preview before the separately approval-gated deploy action. Before a real run, call the read-only run preview, show rows, stages, projected cost, caps, writes, and checkpoint, then use the separately approval-gated start action. Poll with the status action. Resolve a waiting run through the approval-gated decision action; never request or reveal its hook token.
...
- A background dev server does not survive an idle snapshot. Answer a local checkpoint or approval within the same session. A pause which must outlive the session needs a Vercel-hosted workflow deployed through trusted control or, when that control is absent, from a keyboard.
