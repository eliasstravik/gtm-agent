...
export default defineTool({
  ...
  inputSchema: z.object({ query: z.string().nullish(), cursor: z.string().nullish(), limit: z.number().int().min(1).max(100).nullish(), types: z.enum(["public_channel", "private_channel", "im", "mpim"]).nullish() }),
