...
export function describeHost(): string {
  ...
  return [
    ...
    workflow
      ? `The workflow project is connected at ${workflow.GTM_WORKFLOW_URL}. Its keys live in its own settings, which you cannot read; its link route lists their names under \`keys\`, and that list is the only source for anything you say about a key. Its link resolver returns canonical private viewerUrl behind Vercel sign-in. GET /api/link opens the list; GET /api/link/<workflow-id-or-slug> opens a workflow. Explicit sharing uses /api/viewer/service with the same protected host credential and the requested scope and expiry. Its result tables are read through its query route (POST /api/query) with the same credential.`
