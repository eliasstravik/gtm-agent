...
export const allow = {
  ...
  ...(workflow && { [host("GTM_WORKFLOW_URL")]: bearer(workflow.GTM_RUN_SECRET) }),
  "*": [],
};
...
export const exports = workflow ? `export GTM_WORKFLOW_URL=${workflow.GTM_WORKFLOW_URL} GTM_RUN_SECRET=host` : "";
...
export function describeHost(): string {
  ...
  return [
    ...
    workflow
      ? `The workflow project is connected at ${workflow.GTM_WORKFLOW_URL}. Its keys live in its own settings, which you cannot read; its link route lists their names under \`keys\`, and that list is the only source for anything you say about a key. Its result tables are read through its query route (POST /api/query) with the same credential.`
