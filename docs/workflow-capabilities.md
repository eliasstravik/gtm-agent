...
After this agent update, an existing workspace needs the managed recopy described in the vendored deployment reference before using the new helpers. Resolve the target generation from the installed templates. This release adds the `parent_run_key` migration for child batches. Preserve workflow-owned definitions, event sources, and provider adapters. Model/MCP/browser credentials and calendar subscriptions are configured separately in the workflow deployment when a concrete automation is requested.
